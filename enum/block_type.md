...
>[!CAUTION]
>trying to create blocks via `es.CreateMultiBlock()` that goes outside the list of default unlocked items, is count as <i>illegal</i> act of shoplifting or ADs skipper (except for `Sign` and `PotatoGun` ) , and may result in VK account suspension when the MOD is published to VK official "Evertech Sandbox mods" server.
