...
>[!CAUTION]
>trying to create blocks via `es.CreateMultiBlock()` that goes outside the list of default unlocked item, is count as <i>illegal</i> act of shoplifting or ADs skipper, and may result in VK account suspension when the violated MOD is published to VK official "Evertech Sandbox mods" server.
