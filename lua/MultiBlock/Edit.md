...
>[!IMPORTANT]
>if `modUuid` is ommited, and `blockType` match with listed MOD blocks, the block's appearance would show checker patterns with "MOD MISSING" text, and show a blank button when player look at it.